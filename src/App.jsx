...
const pct      = s => s.total > 0 ? Math.min(100, Math.round(((s.paye||0)/s.total)*100)) : 0
...
export default function App() {
  ...
  const studentVersements = id => versements.filter(v=>v.studentId===id)
  ...
  const screens = {
    ...
    pco:         <PCOScreen go={go} logout={logout} students={students} versements={versements} />,
    ...
    etuDash:     <EtuDashScreen go={go} students={students} versements={versements} selectedId={selectedId} studentVersements={studentVersements} />,
    versement:   <VersementScreen go={go} students={students} versements={versements} selectedId={selectedId} studentVersements={studentVersements} />,
  }
      ...
      <div className="scroll">
        <motion.div variants={stagger} initial="initial" animate="animate">
          ...
          <motion.div variants={item} style={{textAlign:'center',marginTop:'1.5rem'}}>
            <button className="admin-link" onClick={()=>go('adminChoice')}>
...
function LoginScreen({ go, loginTarget, setCurrentUser }) {
  ...
  const submit = async () => {
    ...
    if (loginTarget === 'pco') {
      if (id===ADMINS.pco.id && pw===ADMINS.pco.pw) {
        setCurrentUser(ADMINS.pco); go('pco')
      ...
      <div className="scroll">
        ...
        <motion.div variants={stagger} initial="initial" animate="animate">
          <motion.div variants={item} className="card">
            ...
            <button className="btn btn-gold" onClick={submit} disabled={loading} style={{opacity:loading?0.7:1}}>
...
/* ═══════════════════════════════════════════════
   PCO DASHBOARD
═══════════════════════════════════════════════ */
function PCOScreen({ go, logout, students, versements }) {
  ...
  const soldes = sList.filter(s=>s.paye>=s.total).length
  ...
  const addStudent = async () => {
    ...
    const tres = TRES_LOGINS[idx % TRES_LOGINS.length]
    const id = 'custom_'+Date.now()
    await setDoc(doc(db,'students',id),{
      nom:addNom.trim(), ticket:addTicket==='55000'?'Duo':'Solo',
      total:parseInt(addTicket), tresoriere:tres.name, tresPhone:tres.phone,
          ...
          {tab==='dashboard' && (
            <motion.div key="dash" {...slideUp}>
              <div className="stats-grid">
                {[[total,'Inscrits'],[soldes,'Soldés'],[Math.round(collecte/1000)+'k','FCFA collectés'],[pending,'En attente']].map(([n,l])=>(
          ...
          {tab==='students' && (
            <motion.div key="students" {...slideUp}>
              <div className="search-wrap">
                ...
                <input className="search-input" placeholder="Rechercher un étudiant…" value={search} onChange={e=>setSearch(e.target.value)}/>
              </div>
              <motion.div variants={stagger} initial="initial" animate="animate">
                {filtered.slice(0,50).map(s=>{
                  const pc=pct(s), done=s.paye>=s.total
                  return <motion.div key={s.id} variants={item} className="scard" style={{cursor:'default'}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div className="scard-sub">{s.ticket} · {s.tresoriere}</div>
                      <ProgressBar value={pc} done={done}/>
                      <div className="pbar-labels">
                        <span>{fmtF(s.paye||0)}</span>
                        <span style={{color:done?'#86efac':'var(--gold)'}}>{done?'Soldé':fmtF(s.total-(s.paye||0))+' restant'}</span>
                      </div>
                    </div>
                  </motion.div>
                })}
...
function TresScreen({ go, logout, currentUser, students, versements }) {
  ...
  const valider = async vid => {
    ...
    const s = students[v.studentId]
    if(s) await updateDoc(doc(db,'students',v.studentId),{paye:Math.min(s.total,(s.paye||0)+v.montant)})
  }
            ...
            {myPending.map(v=>{
              ...
              return <motion.div key={v.id} variants={item} className="v-item" style={{alignItems:'flex-start',gap:8}}>
                <div style={{flex:1}}>
                  ...
                  <div className="v-meta">{v.date} · Versement #{v.num}</div>
    ...
    <div className="screen">
      <div className="scroll">
        ...
        <motion.div variants={stagger} initial="initial" animate="animate">
          ...
          {q.length>=2 && found.length===0 && <div className="alert alert-warn">Nom non trouvé. Contactez la PCO si vous êtes inscrit(e).</div>}
          <motion.div variants={stagger} initial="initial" animate="animate">
            {found.map(s=>(
              <motion.div key={s.id} variants={item} className="scard" onClick={()=>{setSelectedId(s.id);go('etuPin')}}>
                ...
                <div style={{flex:1}}>
                  <div className="scard-name">{s.nom}</div>
                  <div className="scard-sub">Ticket {s.ticket} · {fmtF(s.total)}</div>
                </div>
...
function EtuPinScreen({ go, students, selectedId }) {
  ...
  const submit = async () => {
    ...
    setBusy(true)
    if(isNew){
      await updateDoc(doc(db,'students',selectedId),{pin})
      go('etuDash')
    } else {
      if(pin===s.pin) go('etuDash')
        ...
        <motion.div variants={stagger} initial="initial" animate="animate">
          <motion.div variants={item} className="card">
            <div style={{textAlign:'center',marginBottom:'1rem'}}>
              ...
              <div style={{fontSize:12,color:'var(--muted)'}}>{isNew ? 'Première connexion — Choisissez votre code PIN à 4 chiffres' : 'Entrez votre code PIN'}</div>
            </div>
            {err && <div className="alert alert-err">{err}</div>}
            <div className="pin-dots">
              {[1,2,3,4].map(i=><div key={i} className={`pin-dot${pin.length>=i?' filled':''}`}/>)}
            </div>
            <input
...
/* ═══════════════════════════════════════════════
   ETU DASHBOARD
═══════════════════════════════════════════════ */
function EtuDashScreen({ go, students, selectedId, studentVersements }) {
  const s = students[selectedId]
  if(!s) return null
  const vers = studentVersements(selectedId)
  const pc   = pct(s)
  const done = (s.paye||0) >= s.total
      ...
      <div className="scroll">
        ...
        <motion.div variants={stagger} initial="initial" animate="animate">
          ...
          <motion.div variants={item} className="card card-gold" style={{textAlign:'center',marginBottom:12}}>
            ...
            <div style={{fontSize:11,color:'var(--muted)',marginTop:4}}>Ticket {s.ticket} · Trésorière&nbsp;: <strong style={{color:'var(--gold-l)'}}>{s.tresoriere}</strong></div>
          </motion.div>
          ...
          <motion.div variants={item} className="card">
            <div className="big-pct-wrap">
              <motion.div
                className={`big-pct${done?' done':''}`}
                initial={{opacity:0,scale:0.5}}
                animate={{opacity:1,scale:1}}
                transition={{duration:0.6,delay:0.2,ease:[0.22,1,0.36,1]}}
              >{pc}%</motion.div>
              <div className="big-pct-sub">de votre ticket réglé</div>
            </div>
            <ProgressBar value={pc} done={done}/>
            <div className="amount-grid" style={{marginTop:'0.75rem'}}>
              <div className="amt-box"><div className="amt-n">{fmtF(s.paye||0)}</div><div className="amt-l">Payé</div></div>
              <div className="amt-box"><div className="amt-n" style={{color:done?'#86efac':'var(--gold)'}}>{fmtF(Math.max(0,s.total-(s.paye||0)))}</div><div className="amt-l">Reste</div></div>
            </div>
            {done
              ? <div className="alert alert-ok" style={{textAlign:'center',marginTop:'0.75rem',fontWeight:500}}>🎉 Ticket entièrement réglé !</div>
          ...
          <motion.div variants={item} className="card">
            <div className="section-title" style={{fontSize:16,marginBottom:'0.75rem'}}>
              Historique&nbsp;<span style={{color:'var(--muted)',fontFamily:'var(--font-body)',fontSize:13,fontWeight:400}}>({vers.length} versement{vers.length>1?'s':''})</span>
            </div>
            ...
            {[...vers].reverse().map(v=>(
              <div key={v.id} className="v-item">
                <div>
                  ...
                  <div className="v-meta">{v.date}</div>
                </div>
                <div style={{textAlign:'right'}}>
                  <div className="v-amt">{fmtF(v.montant)}</div>
...
/* ═══════════════════════════════════════════════
   VERSEMENT
═══════════════════════════════════════════════ */
function VersementScreen({ go, students, selectedId, studentVersements }) {
  ...
  if(!s) return null
  const pc    = pct(s)
  const reste = Math.max(0, s.total - (s.paye||0))
  const amt   = parseInt(montant)||0

  const newTotal = Math.min(s.total, (s.paye||0) + amt)
  const newReste = Math.max(0, s.total - newTotal)
  const newPct   = Math.round((newTotal/s.total)*100)

  const msg = amt>0 ? `Bonjour ${s.tresoriere},\n\nJe suis ${s.nom}, étudiant(e) de la 56ème Promotion de Médecine d'Abidjan.\n\nJe souhaite effectuer mon ${ordinal(nVers)} versement pour le Bal de Promotion — African Red Carpet 2026.\n\n💰 Montant : ${fmtF(amt)}\n📊 Total versé : ${fmtF(newTotal)} / ${fmtF(s.total)} (${newPct}%)\n💳 Reste à payer : ${fmtF(newReste)}\n\nMerci ! 🙏✨` : ''

  const envoyer = async () => {
    if(!amt||amt<=0) return
    setSending(true)
    await addDoc(collection(db,'versements'),{
      studentId: selectedId, montant: amt, num: nVers,
      status: 'pending', tresoriere: s.tresoriere, tresPhone: s.tresPhone,
        ...
        <motion.div variants={stagger} initial="initial" animate="animate">
          ...
          <motion.div variants={item} className="card" style={{marginBottom:12}}>
            <div style={{fontSize:12,color:'var(--muted)',marginBottom:6}}>
              Trésorière assignée&nbsp;: <strong style={{color:'var(--gold-l)'}}>{s.tresoriere}</strong>
            </div>
            <ProgressBar value={pc} done={false}/>
            <div className="pbar-labels">
              <span>{fmtF(s.paye||0)} payés</span>
              <span style={{color:'var(--gold)'}}>{fmtF(reste)} restant</span>
            </div>
          </motion.div>
              ...
              {amt>0 && (
                ...
                >
                  ...
                  <p style={{fontSize:11,color:'var(--muted)',textAlign:'center',marginTop:10,lineHeight:1.6}}>
                    Le versement sera confirmé après validation par la trésorière.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
